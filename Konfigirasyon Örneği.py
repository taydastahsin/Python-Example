...
class TargetConfiguration:
    ...
    def from_configuration_file(self, config_file):
        ...
        if section is None:
            error(
                "%s does not define a section that covers Qt v%s." % (config_file, version_to_string(self.qt_version)))
    ...
    def from_introspection(self, verbose, debug):
        ...
        if sys.platform == 'win32':
            ...
            if dynamic_pylib:
                ...
                elif multiarch != '' and glob.glob(
                        '%s/lib/%s/libpython%d.%d*' % (exec_prefix, multiarch, py_major, py_minor)):
        ...
        if test is None:
            error(
                "Failed to determine the detail of your Qt installation. Try again using the --verbose flag to see more detail about the problem.")
...
''')
    ...
    if not target_config.no_designer_plugin and 'QtDesigner' in target_config.pyqt_modules:
        if generate_plugin_makefile(target_config, verbose, 'designer', target_config.designer_plugin_dir,
                                    "Qt Designer"):
...
def generate_sip_module_code(target_config, verbose, parts, tracing, mname, fatal_warnings, sip_flags, doc_support,
                             qpy_sources=None, qpy_headers=None):
...
def generate_module_makefile(target_config, verbose, mname, include_paths=None, libs='', install_path='', src_dir='',
                             qpy_sources=None, qpy_headers=None):
    ...
    pro_lines.append('CONFIG += warn_on exceptions_off %s' % (
        'staticlib hide_symbols' if target_config.static else 'plugin plugin_bundle'))
